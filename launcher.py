...
mcdir:Minecraft路径
'''
def run(mcdir: str, version: str, javaw_path: str, maxMen: str, username: str, userType: str, uuid: str, access_token: str, i):#启动游戏
    #微软登录
    ...
    if((not javaw_path == "")\
        ...
        if(isMyversion(version, mcdir)):
            ...
            #设置最大运行内存
            JVM = JVM + " " + classpath + " -Xmx" + maxMen + " -Dlog4j.formatMsgNoLookups=true"
            ...
            mc_args = mc_args.replace("${resolution_width}", "854")#窗口宽度
            mc_args = mc_args.replace("${resolution_height}", "480")#窗口高度
            ...
            bat.close()
            system("run.bat")
            #在运行完Minecraft之后，删除run.bat，这样可以避免与其他文件冲突
            remove("run.bat")
    #else:
...
if __name__ == "__main__":
    run(".\\.minecraft", "1.14.4", "C:\\Program Files\\Java\\jdk-19\\bin\\javaw.exe", "1024m", "114514", "Legacy", "{}", "{}",64)
